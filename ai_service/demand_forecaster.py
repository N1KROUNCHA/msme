...
class DemandForecaster:
    ...
    def predict_horizon(self, history: List[float], horizon=7, seq_length=7):
        """
        Predicts multiple days ahead using iterative forecasting.
        """
        if not self.is_trained:
            avg = sum(history[-7:]) / 7
            return [avg] * horizon
        ...
        for _ in range(horizon):
            next_pred = self.predict_next(current_history, seq_length)
            predictions.append(next_pred)
            current_history.append(next_pred)

        return predictions

    def calculate_confidence_intervals(self, predictions: List[float], history: List[float]):
        """
        Calculate confidence intervals based on historical variance.
        """
        historical_std = np.std(history[-30:]) if len(history) >= 30 else np.std(history)
        
        lower_bound = [max(0, p - historical_std) for p in predictions]
        upper_bound = [p + historical_std for p in predictions]
